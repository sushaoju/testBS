          ...
          $(function(){		
			...
			text:'添加到购物车',
  			...
  			$("#grid").datagrid({				
  				...
  				pageSize:5,
  				pageList:[5,10],
  				url:"<%=request.getContextPath()%>/servlet/WDrugServlet?reqCode=findDrugsForPages",
  				columns:[[
  					...
  					{title:"操作",field:"_op",width:160,formatter:function(value,row,index){
  						return '<a href="javascript:void(0);" onclick="sel(\' ' +row.id+ '\')">详情</a>   <a href="javascript:void(0);" onclick = "addcart(\' ' +row.id+ '\')">加入购物车</a>   <a href="javascript:void(0);" onclick = "buy(\' ' +row.id+ '\')">立即购买</a>';
  					}}
  				]]
  			});
  		});
//操作中的快捷方式  		
        function sel(id){
                 alert(id);
                 addSubPage('药品详情','<%=request.getContextPath()%>/zzxy/wdrug_xq.jsp?id=' +id);
           }
  		function addcart(id){
  			addSubPage('加入购物车','<%=request.getContextPath()%>/cart/wcart_add.jsp?id=' +id);
  		}
  		function buy(id){
                 alert(id);
                 addSubPage('立即购买','<%=request.getContextPath()%>/buy/wdrug_buy.jsp?id=' +id);
           }
//重置按钮
  		function clearForm(){
	          $("#22").form('clear');

    ...
    <script type="text/javascript">
          $(function(){		
			...
			text:'添加到购物车',
				iconCls:'icon-add',
				handler:function(){
					addSubPage('添加到购物车','<%=request.getContextPath()%>/servlet/HDrugServlet?reqCode=addcart&id='+id);
				}
			},{
				text:'Cut',
				iconCls:'icon-cut',
  			...
  			$("#grid").datagrid({				
  				...
  				pageSize:10,
  				pageList:[10,20],
  				url:"<%=request.getContextPath()%>/servlet/HDrugServlet?reqCode=findDrugsForPages",
  				columns:[[
  					...
  					{title:"操作",field:"_op",width:200,formatter:function(value,row,index){
  						return '<a href="<%=request.getContextPath()%>/zzxy/hdrug_xq.jsp?id='+row.id+'">药品详情</a>&nbsp<a href="<%=request.getContextPath()%>/cart/hcart_add.jsp?id='+row.id+'">加入购物车</a>&nbsp<a href="<%=request.getContextPath()%>/buy/hdrug_buy.jsp?id='+row.id+'">立即购买</a>';
  					}}
  				]]
  			});
  		});
//操作中的快捷方式  		
        function sel(id){
                 alert(id);
                 addSubPage('药品详情','<%=request.getContextPath()%>/zzxy/hdrug_xq.jsp?id=' +id);
           }
  		function addcart(id){
  			addSubPage('加入购物车','<%=request.getContextPath()%>/cart/hcart_add.jsp?id=' +id);
  		}
  		function buy(id){
                 alert(id);
                 addSubPage('立即购买','<%=request.getContextPath()%>/buy/hdrug_buy.jsp?id=' +id);
           }
//重置按钮
  		function clearForm(){
	          $("#22").form('clear');

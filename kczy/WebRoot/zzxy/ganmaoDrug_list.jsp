    ...
    <script type="text/javascript">
          $(function(){		
			...
			text:'添加到购物车',
				iconCls:'icon-add',
				handler:function (){
			    addSubPage('添加到购物车','<%=request.getContextPath()%>/cart/cart_add.jsp?id=' +id);
				}
			},{
				text:'Cut',
				iconCls:'icon-cut',
  			...
  			$("#grid").datagrid({				
  				...
  				pageSize:5,
  				pageList:[5,10],
  				url:"<%=request.getContextPath()%>/servlet/GDrugServlet?reqCode=findDrugsForPages",
  				columns:[[
  				    {field:"ck",checkbox:true},
  				    {title:"药品ID",field:"id",width:100,align:"center",hidden:false},
  					...
  					return '<a href="<%=request.getContextPath()%>/zzxy/gdrug_xq.jsp?id='+row.id+'">详情</a>&nbsp<a href="<%=request.getContextPath()%>/cart/cart_add.jsp?id='+row.id+'">加入购物车</a>&nbsp<a href="<%=request.getContextPath()%>/buy/gdrug_buy.jsp?id='+row.id+'">立即购买</a>';
...
//操作中的快捷方式  		
        function sel(id){
                 alert(id);
                addSubPage('详情','<%=request.getContextPath()%>/zzxy/gdrug_xq.jsp?id=' +id);
           }
  		function addcart(id){
  			addSubPage('加入购物车','<%=request.getContextPath()%>/cart/cart_add.jsp?id=' +id);
  		}
  		function buy(id){
                 alert(id);
                 addSubPage('立即购买','<%=request.getContextPath()%>/buy/gdrug_buy.jsp?id=' +id);
           }
...
//搜索按钮	        
  ...
  <body>
    <div  style="padding:5px 10px;" id="22">
	   ...
	   <a href="<%=request.getContextPath()%>/cart/cart_list.jsp"  class="easyui-linkbutton" >我的购物车</a>

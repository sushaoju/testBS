          ...
          $(function(){		
			...
			text:'添加到购物车',
  			...
  			$("#grid").datagrid({				
  				...
  				pageSize:5,
  				pageList:[5,10],
  				url:"<%=request.getContextPath()%>/servlet/BDrugServlet?reqCode=findDrugsForPages",
  				columns:[[
  					...
  					{title:"操作",field:"_op",width:160,formatter:function(value,row,index){
  						return '<a href="<%=request.getContextPath()%>/zzxy/bdrug_xq.jsp?id='+row.id+'">详情</a>&nbsp<a href="<%=request.getContextPath()%>/cart/bcart_add.jsp?id='+row.id+'">加入购物车</a>&nbsp<a href="<%=request.getContextPath()%>/buy/bdrug_buy.jsp?id='+row.id+'">立即购买</a>';
  					}}
  				]]
  			});
  		});
//操作中的快捷方式  		
        function sel(id){
                 alert(id);
                 addSubPage('药品详情','<%=request.getContextPath()%>/zzxy/bdrug_xq.jsp?id=' +id);
           }
  		function addcart(id){
  			addSubPage('加入购物车','<%=request.getContextPath()%>/cart/bcart_add.jsp?id=' +id);
  		}
  		function buy(id){
                 alert(id);
                 addSubPage('立即购买','<%=request.getContextPath()%>/buy/bdrug_buy.jsp?id=' +id);
           }
//重置按钮
  		function clearForm(){
	          $("#22").form('clear');

    ...
    <script type="text/javascript">
          $(function(){		
			...
			text:'修改购物车',
				iconCls:'icon-add',
				handler:function(){
			    addSubPage('修改购物车','<%=request.getContextPath()%>/cart/cart_upd.jsp?id=' +id);
				}
			},{
				text:'Cut',
				iconCls:'icon-cut',
  			...
  			$("#grid").datagrid({				
  				...
  				pageSize:5,
  				pageList:[5,10],
  				url:"<%=request.getContextPath()%>/servlet/CartServlet?reqCode=findCartsForPages",
  				columns:[[
  					...
  					{title:"操作",field:"_op",width:160,formatter:function(value,row,index){
  						return '<a href="<%=request.getContextPath()%>/cart/cart_upd.jsp?id='+row.id+'">修改</a>&nbsp<a href="<%=request.getContextPath()%>/servlet/CartServlet?reqCode=delCart&id='+row.id+'">删除</a>&nbsp<a href="<%=request.getContextPath()%>/buy/drug_buy.jsp?id='+row.id+'">立即购买</a>';
  					}}
  				]]
  			});
  		});
//操作中的快捷方式  		
        function upd(id){
                 alert(id);
                addSubPage('详情','<%=request.getContextPath()%>/cart/cart_upd.jsp?id=' +id);
           }

      function del(id){
                alert(id);
                addSubPage('删除','<%=request.getContextPath()%>/servlet/CartServlet?reqCode=delCart&id='+id);
           } 

  		function buy(id){
                 alert(id);
                 addSubPage('立即购买','<%=request.getContextPath()%>/buy/drug_buy.jsp?id=' +id);
           }
  ...
  <body>
  <div class="easyui-panel" title="我的购物车" style="width:95%">
	...
	<div id="grid" style="width:100%;height:350px"></div>
	 ...
	 <script>
          var id=  '<%=request.getParameter("id")%>';	       
	        ...
	        $(function(){	
			$("#ff").form("load","<%=request.getContextPath()%>/servlet/CartServlet?reqCode=findCartById&id="+id);
		});
